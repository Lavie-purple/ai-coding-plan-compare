# -*- coding: utf-8 -*-
"""B 项：生成派生副本 data/_sanitized/ —— 剥离上游数据里的推广内容。

⚠️ 产物**不入库**（已写进 .gitignore）。它是 100% 派生数据，仅 plan-models.json
   单文件就 623,875 字节；提交进仓库等于把 git 当缓存用，还会多一条「副本与正本是否
   同步」的自我维护负担。所以改成**按需生成**：本机要用就跑一次，CI 每次构建现场生成
   并上传为 artifact（保留 14 天）供下载再分发。
