...
> 最新数据截止：**2026-09-14** ｜ 汇率：**1 USD = ¥6.7156**（open.er-api.com 当日中间价）
...
| 文件 | 说明 |
| --- | --- |
| `outputs/AI_Coding_Plan_资费汇总_2026-09-14.html` | 交互式报告，9 个章节、219 条明细。含**搜索**、**价格带 / 性价比档位 / 平台状态三重筛选**、**列排序**、**额度换算器**、**订阅 vs API 盈亏平衡表**、**选型建议**、**平台在售状态总览**，以及**一键下载当日数据表（CSV）** |
| `outputs/AI_Coding_Plan_数据表_2026-09-14.csv` | 219 行 × 14 列结构化数据（UTF-8-BOM，Excel 直接打开不乱码） |
...
CSV 列：`阵营 / 平台 / 套餐 / 平台状态 / 来源评分 / 月费(原币种) / 月费(¥) / 年付折算(¥/月) / 官方用量口径 / 折合Token÷月(测算) / ¥÷百万token(测算) / 性价比档位 / 主力模型 / 备注`

---

## 覆盖范围

- **219 条明细**：国际阵营 67 · 国内阵营 129 · API 基准 23
- **43 家平台**的在售状态与官方评级
...
订阅档层面（国内 + 国际共 196 档）：在售 109 · 限量 18 · 暂停 6 · 已下架 22 · 未收录 41。
...
3. **折算覆盖率有限。** 平台级在售的 109 个订阅档中只有 56 档（51%）能折算为 token；其余采用积分 / Credits / AFP / prompt 计量且官方未给倍率表，**一律留空而不臆造**。CSV 中这些行的 token 与单价列为空，但保留「官方用量口径」原始单位。
4. **单价区间**
   - 平台级在售档：¥0.0661 ~ ¥2.50 / 百万 token（38 倍）
   - 含已下架历史档：¥0.0165 ~ ¥6.0916 / 百万 token（370 倍）
5. **同一订阅跨模型可差 30 倍。** 例：Codex Pro×20（$200）在 GPT-5.6-Luna 口径下约 960 亿 token/月（¥0.0143/M），换 GPT-6-Astra 仅约 32 亿 token/月（¥0.425/M）。**选型必须先锁定目标模型。**
