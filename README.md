...
| 文件 | 说明 |
| --- | --- |
| `outputs/AI_Coding_Plan_资费汇总_<日期>.html` | 交互式报告，10 个章节、218 条明细。含**三套可切换皮肤**、**七天回看**、**搜索**、**价格带 / 性价比档位 / 平台状态三重筛选**、**列排序**、**额度换算器**、**订阅 vs API 盈亏平衡表**、**选型建议**、**平台在售状态总览**、**日环比区块**，以及**一键下载当日数据表（CSV）** |
...
fetch_data.py  →  build_report.py  →  tools/verify_output.py  →  git push
   （取数+哈希比对）    （算价+出产物）      （77 项核验）         （本地 pre-commit 先拦一道）
```
...
3. **核验**：`tools/verify_output.py` 跑 77 项断言后放行；`tools/test_history.py` 另跑一遍合成多日窗口的端到端测试。
...
│   ├── verify_output.py       #   产物核验 77 项（含前端现算 CSV 逐字节比对、七天回看逐日还原、hidden 复位）
│   ├── test_history.py        #   七天回看端到端测试 31 项：临时目录合成 5 天快照，跑真实构建并核对
