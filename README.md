...
3. **核验**：`tools/verify_output.py` 跑 226 项断言后放行；`tools/test_history.py` 另跑一遍合成多日窗口的端到端测试（43 项）。注意 226 是**单快照窗口**的基准值 —— 回看窗口里一旦有了历史快照，`[12]` 会多出一条「逐日还原与磁盘 CSV 逐行比对」，于是实际项数为 227（这条断言只在多日窗口才有意义，单快照时无历史可比）。净化副本是**派生数据、不入库**：CI 每次现场生成 `data/_sanitized/` 并抽检（要一份可再分发的干净数据时手动跑 `tools/make_sanitized.py` 即可）。
