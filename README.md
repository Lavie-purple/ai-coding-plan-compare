...
| 文件 | 说明 |
| --- | --- |
| `outputs/AI_Coding_Plan_资费汇总_<日期>.html` | 交互式报告，10 个章节、218 条明细。含**三套可切换皮肤**、**七天回看**、**搜索**、**价格带 / 性价比档位 / 平台状态三重筛选**、**自定义排序**（表头一键排 + 可叠加多级规则 + 一键预设；枚举列按语义序、缺值恒排末位、规则记在本机浏览器里）、**额度换算器**、**订阅 vs API 盈亏平衡表**、**选型建议**、**平台在售状态总览**、**日环比区块**，以及**一键下载当日数据表（CSV）** |
...
3. **核验**：`tools/verify_output.py` 跑 112 项断言后放行；`tools/test_history.py` 另跑一遍合成多日窗口的端到端测试（41 项）。
...
│   ├── verify_output.py       #   产物核验 112 项（含前端现算 CSV 逐字节比对、七天回看逐日还原、hidden 复位、排序规则链）
